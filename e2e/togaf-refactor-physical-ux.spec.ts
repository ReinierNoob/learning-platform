...
test('TOGAF RC1 authenticated lessons, practice, retries and mobile', async ({ browser }) => {
  ...
  try {
    ...
    await mobile.close();
    await navigate(page, module5PracticePath);
    await page.getByRole('checkbox', { name: 'Gebruik mijn opgeslagen uitwerking uit deze module voor dit gesprek' }).check();
    await page.getByRole('textbox', { name: 'Je vraag aan Alexander' }).fill('Waarom is een betaalopdracht nog geen ontvangen waarde voor de verzekerde? Leg het uit met een concreet voorbeeld uit deze module.');
    const tutorResponse = page.waitForResponse(r => r.url().endsWith('/api/chat/5'), { timeout: 90000 });
    await page.getByRole('button', { name: 'Verstuur', exact: true }).click();
    const tutor = await tutorResponse;
    expect(tutor.status(), 'Live tutor availability').toBe(200);
    const tutorBody = await tutor.json();
    expect(typeof tutorBody.reply).toBe('string');
    expect(tutorBody.reply.length).toBeGreaterThan(80);
    await expect(page.locator('.bubble.assistant')).toBeVisible();
    checks.push('live tutor responds with saved-work opt-in; semantic quality not assessed by this test');
    const draft = page.getByRole('textbox', { name: 'Je model en onderbouwing' });
    await expect(draft).toBeEnabled();
    await draft.fill('Concept dat behouden moet blijven wanneer toegang verloopt.');
    const oidcRequest = process.env.ACTIONS_ID_TOKEN_REQUEST_URL;
    const oidcRequestToken = process.env.ACTIONS_ID_TOKEN_REQUEST_TOKEN;
    expect(Boolean(oidcRequest && oidcRequestToken)).toBe(true);
    const oidc = await fetch(`${oidcRequest}&audience=eaw-learning-platform-ux-e2e`, { headers: { Authorization: `Bearer ${oidcRequestToken}` } });
    expect(oidc.ok).toBe(true);
    const oidcBody = await oidc.json();
    const expired = await fetch('https://mhjykzrljvtxauaatlom.supabase.co/functions/v1/github-togaf-ux-e2e-bootstrap', { method: 'POST', headers: { Authorization: `Bearer ${oidcBody.value}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'expire', user_id: process.env.EAW_UX_USER_ID, order_id: process.env.EAW_UX_ORDER_ID, order_item_id: process.env.EAW_UX_ORDER_ITEM_ID, entitlement_id: process.env.EAW_UX_ENTITLEMENT_ID }) });
    expect(expired.status, 'Expire only the OIDC-owned test entitlement').toBe(200);
    const expiredSave = page.waitForResponse(r => r.url().includes('/api/practice/5?') && r.request().method() === 'POST');
    await page.getByRole('button', { name: 'Uitwerking opslaan', exact: true }).click();
    expect((await expiredSave).status()).toBe(403);
    await expect(draft).toHaveValue('Concept dat behouden moet blijven wanneer toegang verloopt.');
    await expect(page.locator('.practiceWork [role=status]')).toContainText('Opslaan is niet gelukt');
    await navigate(page, `/leren/${slug}`);
    await expect(page.locator('.hero')).toContainText('Geen actieve toegang');
    checks.push('real entitlement expiry denies new writes and preserves unsaved draft');
