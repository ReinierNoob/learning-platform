...
export async function POST(request: Request) {
  ...
  const response = await fetch(`${supabaseUrl}/auth/v1/signup`, {
    ...
    body: JSON.stringify({ email: String(email ?? "").trim().toLowerCase(), password, data: { first_name: firstName, last_name: lastName }, email_redirect_to: `${origin}/inloggen?geverifieerd=1` }),
  });
