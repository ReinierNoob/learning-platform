...
import { module5ClassifierVersion, module5OrchestratorVersion, observeModule5Reasoning } from "../../../../../lib/solution-architecture-module-5-runtime";
...
export async function POST(request: Request) {
  ...
  const intervention = solutionArchitectureModule5.interventions[interventionId];
  if (!intervention?.prompt) return NextResponse.json({ error: "observation_not_supported" }, { status: 400 });
  const observation = observeModule5Reasoning(interventionId, body?.answer);
    ...
    try {
      ...
      const persisted = await persistAdaptiveTransitionForLearner(context, {
        profile: {
          ...
          routeState: { ...(state.profile?.route_state ?? {}), module: adaptiveModule5SourceModuleId, phase: "tutor_observation", interventionId, observationLevel: observation.level },
        ...
        decision: {
          ...
          reasonCode,
          rationale: `Deterministic Module 5 tutor observation for ${interventionId}`,
      ...
      console.error("adaptive_module5_observation_persistence_failed", error);
