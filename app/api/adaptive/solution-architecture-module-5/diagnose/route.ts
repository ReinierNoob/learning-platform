...
import { adaptiveAccessStatus, adaptiveSchemaVersion, adaptiveSolutionArchitectureCourseSlug, adaptiveModule5SourceModuleId, isAdaptivePersistenceEnabled } from "../../../../../lib/adaptive-runtime";
import { AdaptiveAccessError, persistAdaptiveTransitionForLearner, requireAdaptiveLearningContext } from "../../../../../lib/adaptive-service";
import { diagnoseModule5Answers, module5ClassifierVersion, module5OrchestratorVersion } from "../../../../../lib/solution-architecture-module-5-runtime";
...
export async function POST(request: Request) {
  ...
  const diagnosis = diagnoseModule5Answers(body?.answers ?? {});
    ...
    try {
      ...
      const persisted = await persistAdaptiveTransitionForLearner(context, {
        profile: {
          ...
          routeState: { module: adaptiveModule5SourceModuleId, route: diagnosis.route, reasonCode: diagnosis.reasonCode, phase: "diagnosis" },
          preferences: {},
        },
        ...
        decision: {
          ...
          rationale: `Module 5 diagnosis selected route ${diagnosis.route}`,
      ...
      console.error("adaptive_module5_diagnosis_persistence_failed", error);
      return NextResponse.json({ error: "adaptive_persistence_failed" }, { status: 500 });
    }
  }

  return NextResponse.json({ ...diagnosis, profile: { conceptMastery: diagnosis.mastery, persistence, transitionIds } });
}
