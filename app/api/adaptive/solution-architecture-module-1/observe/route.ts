...
import { module1ClassifierVersion, module1OrchestratorVersion, observeModule1Reasoning } from "../../../../../lib/solution-architecture-module-1-server";
...
export async function POST(request: Request) {
  ...
  const intervention = solutionArchitectureModule1.interventions[interventionId];
  if (!intervention?.prompt) return NextResponse.json({ error: "observation_not_supported" }, { status: 400 });
  const observation = observeModule1Reasoning(interventionId, body?.answer);
    ...
    try {
      ...
      const persisted = await persistAdaptiveTransitionForLearner(context, {
        profile: {
          ...
          routeState: { ...(state.profile?.route_state ?? {}), module: adaptiveModule1SourceModuleId, phase: "tutor_observation", interventionId, observationLevel: observation.level },
        ...
        decision: {
          ...
          reasonCode,
          rationale: `Deterministic Module 1 tutor observation for ${interventionId}`,
      ...
      console.error("adaptive_module1_observation_failed", error);
