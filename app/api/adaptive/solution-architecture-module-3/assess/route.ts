...
import { module3AnswerKey, module3AssessmentVersion, module3OrchestratorVersion, module3RemediationByQuestion } from "../../../../../lib/solution-architecture-module-3-server";
...
export async function POST(request: Request) {
  ...
  const results = solutionArchitectureModule3.assessment.map((question) => ({ id: question.id, objectiveId: question.objectiveId, correct: answers[question.id] === module3AnswerKey[question.id] }));
  const correct = results.filter((item) => item.correct).length;
  const passed = correct === results.length;
  const remediationSequence = Array.from(new Set(results.flatMap((item) => item.correct ? [] : module3RemediationByQuestion[item.id] ?? [])));
  ...
  if (isAdaptivePersistenceEnabled()) {
    try {
      context = await requireAdaptiveLearningContext(adaptiveSolutionArchitectureCourseSlug, adaptiveModule3SourceModuleId);
      ...
      const persisted = await persistAdaptiveTransitionForLearner(context, {
        profile: {
          ...
          classifierVersion: module3AssessmentVersion,
          conceptMastery: { ...(state.profile?.concept_mastery ?? {}), ...profileUpdate },
          misconceptionSignals: state.profile?.misconception_signals ?? {},
          routeState: { ...(state.profile?.route_state ?? {}), module: adaptiveModule3SourceModuleId, phase: passed ? "mastery_check_passed" : "assessment_remediation", assessment: { correct, total: results.length, passed, remediationSequence } },
          preferences: state.profile?.preferences ?? {},
        },
        evidence: results.map((item) => ({ moduleId: context!.module.id, objectiveId: item.objectiveId, evidenceType: "assessment" as const, sourceRef: item.id, result: { correct: item.correct }, evidenceStrength: 1, classifierVersion: module3AssessmentVersion })),
        decision: { moduleId: context.module.id, objectiveId: passed ? null : results.find((item) => !item.correct)?.objectiveId ?? null, action: passed ? "recheck" : "targeted_remediation", routeId: passed ? "complete" : "remediation", selectedContentIds: passed ? [] : remediationSequence, reasonCode: passed ? "ASSESSMENT_MASTERED" : "ASSESSMENT_REMEDIATION", rationale: passed ? "All mandatory Module 3 mastery checks passed" : "One or more mandatory Module 3 mastery checks require targeted remediation", orchestratorVersion: module3OrchestratorVersion, learnerOverride: false },
      });
      ...
      console.error("adaptive_module3_assessment_failed", error);
  ...
  if (syncPlatformProgress && passed) {
    try {
      context ??= await requireAdaptiveLearningContext(adaptiveSolutionArchitectureCourseSlug, adaptiveModule3SourceModuleId);
      platformProgress = await syncAdaptiveModulePlatformProgress(context, solutionArchitectureModule3.assessment, module3AnswerKey, answers);
    } catch (error) {
      if (!(error instanceof AdaptiveAccessError)) console.error("adaptive_module3_platform_progress_failed", error);
