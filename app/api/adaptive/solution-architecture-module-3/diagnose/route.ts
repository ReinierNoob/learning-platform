...
import { diagnoseModule3, module3ClassifierVersion, module3OrchestratorVersion } from "../../../../../lib/solution-architecture-module-3-server";
...
export async function POST(request: Request) {
  ...
  const diagnosis = diagnoseModule3(body?.answers ?? {});
    ...
    try {
      ...
      const persisted = await persistAdaptiveTransitionForLearner(context, {
        profile: {
          ...
          routeState: { module: adaptiveModule3SourceModuleId, route: diagnosis.route, reasonCode: diagnosis.reasonCode, phase: "diagnosis" },
          preferences: {},
        },
        evidence: diagnosis.evidence.map((item) => ({ moduleId: context.module.id, objectiveId: item.objectiveId, evidenceType: "diagnostic" as const, sourceRef: item.id, result: { passed: item.passed }, evidenceStrength: item.passed ? 0.85 : 0.55, classifierVersion: module3ClassifierVersion })),
        decision: { moduleId: context.module.id, objectiveId: null, action: diagnosis.route === "A" ? "full_route" : diagnosis.route === "B" ? "accelerated_route" : "targeted_remediation", routeId: diagnosis.route, selectedContentIds: diagnosis.sequence, reasonCode: diagnosis.reasonCode, rationale: `Module 3 diagnosis selected route ${diagnosis.route}`, orchestratorVersion: module3OrchestratorVersion, learnerOverride: false },
      });
      ...
      console.error("adaptive_module3_diagnosis_persistence_failed", error);
