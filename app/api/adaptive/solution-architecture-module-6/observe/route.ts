...
import { observeModule6Reasoning } from "../../../../../lib/module6-tutor-observation";
...
export async function POST(request: Request) {
  ...
  const observation = observeModule6Reasoning(interventionId, body?.answer);
  if (!observation) {
  ...
  if (isAdaptivePersistenceEnabled()) {
    try {
      ...
      const action = observation.level === "strong"
        ? "challenge"
        : observation.level === "partial"
          ? "deeper_explanation"
          : "extra_practice";
      const reasonCode = observation.level === "strong"
        ? "TUTOR_OBSERVATION_STRONG"
        : observation.level === "partial"
          ? "TUTOR_OBSERVATION_PARTIAL"
          : "TUTOR_OBSERVATION_NEEDS_WORK";
      ...
      const persisted = await persistAdaptiveTransitionForLearner(context, {
        ...
        evidence: [{
          ...
          // Privacy: persist derived pedagogical signals, never the learner's free text.
          result: {
