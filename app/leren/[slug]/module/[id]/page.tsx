...
export default async function ModulePage({ params }: { params: Promise<{ slug: string; id: string }> }) {
  ...
  if (!Number.isInteger(sourceModuleId) || sourceModuleId < 1) return <main className="shell"><section className="hero"><h1>Module niet gevonden.</h1></section></main>;
  ...
  if (!module) return <main className="shell"><section className="hero"><p className="eyebrow">Nog niet beschikbaar</p><h1>Deze module is nog niet gepubliceerd.</h1><a className="button" href={`/leren/${slug}`}>Terug naar de training</a></section></main>;
  ...
  return <main className="shell">
    ...
    <LearningClient moduleId={module.source_module_id} questions={module.quiz ?? []}/>
    {(module.chapters ?? []).map((chapter) => <section className="chapter" key={chapter.id}><h2>{chapter.titel}</h2><video controls preload="metadata" src={`/api/video-url/${module.source_module_id}/${encodeURIComponent(chapter.id)}`}/><ReactMarkdown>{chapter.tekst}</ReactMarkdown></section>)}
