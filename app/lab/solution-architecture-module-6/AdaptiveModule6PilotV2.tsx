...
export default function AdaptiveModule6PilotV2() {
  ...
  const promptComplete = !intervention.prompt || promptAnswer.trim().length >= 3;
  ...
  return <div className={styles.shell}>
    <aside className={styles.sidebar}>
      ...
      {customSequence ? <button className={styles.fullRoute} onClick={() => { setCustomSequence(null); setRouteOverride(null); setStepIndex(0); setMaxReached(0); setAssessment(null); setAssessmentAnswers({}); focusLearningStep(); }} type="button">Terug naar mijn leerroute</button> : null}
    </aside>
    ...
    <main className={styles.main}>
      ...
      <section className={styles.learningArea}>
        ...
        <div className={styles.lessonPanel}>
          ...
          {intervention.prompt ? <div className={styles.prompt}>
            ...
            <label><span>Jouw redenering</span><textarea rows={4} value={promptAnswer} onChange={(event) => setPromptResponses((current) => ({ ...current, [intervention.id]: event.target.value }))} placeholder="Schrijf kort op hoe je tot je antwoord komt…" /></label>
          </div> : null}
      ...
      <footer className={styles.controls}>
        <button disabled={stepIndex === 0 || submitting} onClick={previousStep} type="button">Vorige</button>
        <span aria-live="polite">Stap {stepIndex + 1} van {sequence.length}</span>
        <button disabled={!canGoNext || submitting} onClick={nextStep} type="button">Volgende</button>
      </footer>
    </main>
  </div>;
}
